...
export function useSecurityQuery<T>(resource: string, enabled = true) {
  const { user } = useAuth();
  return useQuery({ queryKey: ['security', user?.id, resource], queryFn: () => api<T>(`/${resource}`), enabled: !!user && enabled, retry: 1, staleTime: 10000, refetchInterval: resource === 'dashboard' || resource === 'alerts' || resource.startsWith('support/') ? 30000 : false });
}
